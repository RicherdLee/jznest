...
class activity
{
	...
	function save_activity_product()
	{
		...
		if(!empty($_POST['chk']))
		{
			...
			{
				$sql="select id from ".ACTIVITYPRODUCT." where product_id='$val' and activity_id=$id";
				$this->db->query($sql);
				$aid=$this->db->fetchField('id');
				if(!$aid)
				{
					...
					$sql="INSERT INTO ".ACTIVITYPRODUCT."(`activity_id` ,`product_id` ,`product_name` ,`member_id` ,`member_name` ,`status` ,`create_time`) VALUES ('$id', '$val', '$pname', '0', '', '2','".time()."')";
