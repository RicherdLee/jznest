<?php
	...
	if($_GET['operation']=="add" or $_GET['operation']=="edit")
	{
		...
		{	
			...
			{	
				...
				foreach(explode("\r\n",$_POST['cat']) as $catv)
				{
					if(!empty($catv))
					{
						$sql="insert into ".PCAT." (`catid`,`cat`,`nums`,`isindex`,`pic`,`brand`,ext_table,ext_field_cat,commission,is_setmeal,isbuy,service,current,templates) values 
							('$id','$catv','255','$_POST[isindex]','','$_POST[brand]','$ext_table','$_POST[ext_field_cat]','$_POST[commission]','$_POST[is_setmeal]','$_POST[isbuy]','$_POST[service]','$_POST[current]','$_POST[templates]')";
						$db->query($sql);
						$id+=1;
					}
				}
			...
			if($_POST["act"]=='edit' and is_numeric($_POST['id']))
			{
				...
				$sql="update ".PCAT." set catid='$id', cat='$_POST[cat]',isindex='$_POST[isindex]' ,pic='',isbuy='$_POST[isbuy]',brand='$_POST[brand]',ext_table='$ext_table',ext_field_cat='$_POST[ext_field_cat]',commission='$_POST[commission]',is_setmeal='$_POST[is_setmeal]',service='$_POST[service]',current='$_POST[current]',templates='$_POST[templates]' where catid='".$_POST['id']."'";
				$db->query($sql);
		...
		if($_GET['editid'] and is_numeric($_GET['editid']))
		{
			...
			$re['services']=explode(',',$re['service']);
			$tpl->assign("re",$re);
		}
