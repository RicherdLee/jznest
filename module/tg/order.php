...
if($buid&&$_GET['id']*1>0)
{
	...
	if($config['temp']=='wap')
	{
		$tpl->assign("addr",$addr=$orderadder->get_default_orderadder());
	}
	else
	{
	...
	if(!empty($_POST))
	{
		...
		$address["tel"]=$address["tel"]?$address["tel"]:$address["mobile"];
		
		$sql="insert into ".TGORDER." set order_id='$order_id',is_virtual='$de[is_virtual]', 	company_id='$de[member_id]',tg_name='$de[name]',tg_id='$de[id]',tg_pic='$de[pic]',status='20',member_id='$buid',member_name='$b2bbuilder_auth[1]',contact='$address[name]',address='$address[area] $address[address]',tel='$address[tel]',remark='$_POST[remark]',price ='$de[price]',quantity ='$_POST[quantity]',create_time='$tm'";
		$re=$db->query($sql);
