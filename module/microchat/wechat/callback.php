        ...
        if (!empty($postStr) or $debug == 1){
            ...
            if($setp > 0 and $setp < 3){
                ...
                if($keyword == 'quit'){
					...
					if(empty($oldname)){
					$db -> query("UPDATE ".$weixinusertable." SET `uname` = '$olduname' WHERE `wxid`= '$fromUsername';");
					}
                ...
                if($setp == 1){
                    ...
                    if(empty($ret)){
					    ...
					    $oldusername = $db -> getRow("SELECT `uname` FROM ".$weixinusertable." WHERE `wxid` = '$fromUsername'");
                        $olduname = $oldusername['uname'];
						$newuname = $keyword;
						$db -> query("UPDATE ".$weixinusertable." SET `setp`=`setp`+1 WHERE `wxid`= '$fromUsername';");
						$db -> query("UPDATE ".$weixinusertable." SET `uname` = '$newuname' WHERE `wxid`= '$fromUsername';");
						$contentStr = '请输入帐号：'.$newuname.'的密码';
                    }
                }elseif($setp == 2){
                    $password = $keyword;
            ...
            if($keyword == 'debug'){
				...
				$db -> query("UPDATE ".$weixinusertable." SET `setp` = 0 WHERE `wxid`= '$fromUsername';");
                    $contentStr = '您已进入会员绑定流程，想要退出绑定流程请回复【quit】,继续请输入网站会员帐号';
                    $db -> query("UPDATE ".$weixinusertable." SET `setp`=`setp`+1 WHERE `wxid`= '$fromUsername';");
                $msgType = "text";
                $resultStr = sprintf($textTpl, $fromUsername, $toUsername, $time, $msgType, $contentStr);
                echo $resultStr;
