...
<div class="bigbox">
	...
	<div class="bigboxbody">
	  <table width="100%" border="0" cellpadding="2" cellspacing="0" >
        <tr class="theader">
          ...
          <td width="*" ><?php echo lang_show('payment_b');?></td>
          <td width="11%" align="center" >开启状态</td>
	...
	for($i=0;$i<$coun_num;$i++)
	{
		...
		if ($payment_one) {
			...
			$payment_type = $payment_one['payment_type'];
		} else {
	...
	?>
        <tr  onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this,'odd')">
          ...
          <td align="center"><?php echo $active?'启用':'停止';?></td>
          <td align="center">
		  <?php
				echo '<a href="module.php?m=payment&s=paymentmod.php&step=edit&payment_type='.$payments[$i]["payment_name"].'">编辑</a>';
				echo " | ";
				if($active==0) {
					echo '<a href="module.php?m=payment&s=paymentmod.php&step=active&payment_type='.$payments[$i]["payment_name"].'">'.$startimg.'</a>';
				} else {
					echo '<a onClick="return confirm(\'确认关闭吗\')" href="module.php?m=payment&s=payment.php&step=unset&payment_type='.$payments[$i]["payment_name"].'">'.$stopimg.'</a> ';
				}
		?>	 </td>
          <?php 
    	}
