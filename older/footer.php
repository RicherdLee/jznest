...
$sql="select * from ".WEBCON." where con_statu=1 and con_group=0 and lang='$config[language]' order by con_no asc";
$db->query($sql);
while($v=$db->fetchRow())
{
...
{
	$config['copyright'].='<br />Powered by <a href="http://www.hzsoudu.cn">SOUDO Co.,Ltd.</a>';
...
{
	$searcharray[] = "/localhost\/shop/";
	$searcharray[] = "/www\.hzsoudu\.cn/";
	$searcharray[] = "/localhost\/mallbuilder/";
	$replacearray[] = "cn.shop-builder.cn";
	$replacearray[] = "cn.shop-builder.cn";
	$replacearray[] = "cn.shop-builder.cn";
