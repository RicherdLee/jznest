...
if(!empty($_POST["Submit"])&&empty($_GET["edit"])&&$add_type==1)
{
	...
	$sql="insert into ".DOMAIN." 
	(`dtype`,`domain`,top_domain,`con`,`con2`,`con3`,`des`,logo,web_title,web_keyword,web_des,copyright,template) 
	values 
	('1','$p[domain]','$p[top_domain]','$p[province]','$p[city]','$p[area]','$p[des]','$p[logo]','$p[web_title]','$p[web_keyword]','$p[web_des]','$p[copyright]','$p[template]')";
...
if(!empty($_POST["Submit"])&&!empty($_GET["edit"]))
{
	...
	$sql="update ".DOMAIN." set `domain`='$p[domain]',`top_domain`='$p[top_domain]',`con`='$p[province]',`con2`='$p[city]',`con3`='$p[area]',`des`='$p[des]',`isopen`='$p[isopen]',web_title='$p[web_title]',web_keyword='$p[web_keyword]',web_des='$p[web_des]',copyright='$p[copyright]',template='$p[template]',logo='$p[logo]' where id='$_GET[edit]'";
	$re=$db->query($sql);
	if($re)
		msg("sub_domain_list.php");
}
