...
<div class="bigbox">
  <div class="bigboxbody">
    <?php
		$sql="select a.user,a.lastLoginTime,a.email,a.name as contact from ".MEMBER." a  where a.email<>'' $sqlw order by a.userid desc limit ".$limits.",5";
		$limits=$limits+5;
	    $db->query($sql);
	    $re=$db->getRows();
        if($db->num_rows()>0)
        {
			...
			//	$mcontent=str_replace('[company]',$v['company'],$mcontent);
			 ...
			 else
			 {
				...
				//操作记录入库
                 $sql="select a.user,a.lastLoginTime,a.email,a.name as contact from ".MEMBER." a where a.email<>'' $sqlw ";
                 $db->query($sql);
		         $re=$db->getRows();
		         $mrecord='';
	             foreach($re as $v)
		 ...
		 {
             $sql="select count(*) as totalsend from ".MEMBER." a where a.email<>'' $sqlw ";
             $db->query($sql);
			 $re=$db->fetchRow();
