...
function getChildAreaHtml(fid,level){
	...
	if (idName){
	   ...
	   getAreaList(provinceCityJson[fid+""],getIdNameByLevel(level),level);
    }
}
/**
新地址列表数据
...
(function(){
    ...
    JdStockTabs.bind('click',function(){
        ...
        JdStockTabs.removeClass("curr").eq(index).addClass("curr");
        JdStockTabs.find("a").removeClass("hover").eq(index).addClass("hover");
        JdStockContents.hide().eq(index).show();
    });
