...
function iframe_form(id, title, _url, width, height,fresh) {
    ...
    var d = DialogManager.create(id);
    d.setTitle(title);
    var _html = "<iframe id='iframe_"+id+"' src='" + _url + "' width='" + width + "' height='" + height + "' frameborder='0'></iframe>";
    d.setContents(_html);
    d.setWidth(width+2);
    ...
    'createContents'  : function(type, options){
        ...
        switch(type){
			...
			case 'ajax_notice':
                ...
                $.get(options, function(data) {
                var json = eval('(' + data + ')');
                var MsgTxt = '<div class="dialog_message_body clearfix"></div><div class="dialog_message_contents dialog_message_notice">' + json.Msg + '</div><div class="dialog_buttons_bar"></div>'
            ...
            case 'message':
                ...
                _message_body = $('<div class="dialog_message_body clearfix"></div>');
                _message_contents = $('<div class="dialog_message_contents dialog_message_' + type + '"><span></span>' + options.text + '</div>');
