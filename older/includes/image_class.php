...
class cls_image
{
    ...
    function make_thumb($img, $path = '',$thumb_width = 0, $thumb_height = 0,  $bgcolor='')
    {
		...
		if(!$thumb_width or $thumb_width=='')
		{
			$thumb_width=$org_info[0];
		}
		if(!$thumb_height or $thumb_height=='')
		{
		...
		if($this->watermark)
		{
			if($thumb_width>400 && (isset($watermark_config['wmark_type']) && $watermark_config['wmark_type']!=0))
			{
				...
				{
					$watermark_config['wmark_image']=$watermark_config['wmark_image']?$watermark_config['wmark_image']:'logo.gif';
					$water_img=$config['webroot']."/image/default/".$watermark_config['wmark_image'];
