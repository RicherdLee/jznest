...
if(!empty($_POST["action"]))
{
	...
	$sql="select userid,email,user from ".MEMBER." where userid='$_GET[userid]' ";
	$db->query($sql);
	$uid=$db->fetchRow();
	if(!empty($_POST['password'])) 
	{
		$sql="update ".MEMBER." set password='".md5($_POST['password'])."' where userid='$_GET[userid]'";
		$db->query($sql);
	}
	$sql="update ".MEMBER." set statu='$_POST[statu]',point='$_POST[point]',regtime='$_POST[regtime]',email='$_POST[email]' where userid='$_GET[userid]'";
	$db->query($sql);
...
<div class="bigbox">
	...
	<div class="bigboxbody">
      <form method="post" action=""  name=cityform>
		<input name="point" type="hidden" value="<?php if(!empty($point)) echo $point; else echo 0;?>">			
