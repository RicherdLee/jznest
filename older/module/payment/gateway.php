...
$type=$_GET['type'];			//支付类型，1为直接到账，2为担保支付
$module=$_GET['module']?$_GET['module']:"product";
...
switch ($action)
{
	case "add":
	{
		...
		if($module=='product')
			msg("$config[weburl]/main.php?m=product&s=admin_order_pay&order_id=$order_id");//转向支付页面
		else
	...
	case "update":
	{
		if($statu==0)
		{
			...
			if($ss['statu']>=2)
			{
				$price=$ss['price']<0?$ss['price']*-1:$ss['price']*1;
				$sql = "update ".PUSER." set cash=cash+$price where email='$buyer_email'";
				$re=$db->query($sql);
		...
		if($statu==4)
		{
			...
			$sql = "update ".PUSER." set unreachable=unreachable-$price where email='$buyer_email'";
			$re=$db->query($sql);
		}
		if($statu==5)
		{
