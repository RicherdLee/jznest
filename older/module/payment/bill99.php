...
<?PHP
	...
	$payment=new payment();
	$configs=$payment->get_pay_config('bill99');
	
	if(is_numeric($_GET['o']))
	$extra_common_param=$_GET['o'];
	
	//人民币网关账号，该账号为11位人民币网关商户编号+01,该参数必填。
	$merchantAcctId = $configs['bill99_account']; 
	//编码方式，1代表 UTF-8; 2 代表 GBK; 3代表 GB2312 默认为1,该参数必填。
	$inputCharset = "1";
	//接收支付结果的页面地址，该参数一般置为空即可。
	$pageUrl = "";
	//服务器接收支付结果的后台地址，该参数务必填写，不能为空。
	$bgUrl = $config["weburl"]."/main.php?m=payment&s=admin_accounts_base&onlinepaytype=bill99".$u;
	//网关版本，固定值：v2.0,该参数必填。
	$version =  "v2.0";
	...
	/////////////  RSA 签名计算 ///////// 开始 //
	$fp = fopen($config['webroot']."/module/payment/lib/bill99/99bill-rsa.pem", "r");
...
<body>
<form name="kqPay" id="kqPay" action="https://sandbox.99bill.com/gateway/recvMerchantInfoAction.htm" method="post">
