<?php
			...
			if($_POST['group'])
			{
				...
				{		
					...
					{	
						...
						if($val and $group)
						{							
							$sql = "insert into ".PROPERTYVALUE." (`field`,field_name,field_desc,display_type,item,is_search,property_id,statu,type,displayorder) values ('field','$val','$group','1','','0','$id','1','3','0')";
							$db->query($sql);
							$sid=$db->lastid();
							$c[]=$sid;
	...
	if($_GET['editid'] and is_numeric($_GET['editid']))
	{
		...
		$sql="select * from ".PROPERTYVALUE." where property_id='$_GET[editid]' order by displayorder,id";
		$db->query($sql);
		$re=$db->getRows();
