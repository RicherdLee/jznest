...
if(!$cache->begin(array($id,'','',$_GET['catid']),3600*0))
{
	...
	{
		$re[$key]['picName']=str_replace('localhost/shop','cn.a5shop.cn',$val['picName']);	
	}
	if($ad_re['ad_type']==1)
	{	
		//幻灯
...
function get_add($re,$ar)
{
	...
	{
		$re['picName']=str_replace('a5shop','shop-builder',$re['picName']);
		if($re["url"])
			$str="<a href=".$re['url']." target=_blank><img src=".$re['picName']." border=0  height=".$height." width=".$width."></a>";
		else
			$str="<img src=".$re['picName']." height=".$height." width=".$width.">";
...
function jside($re,$ar,$type)
{
	...
	if(!empty($re))
	{
		foreach($re as $k=>$v)
		{			
			$v['picName']=str_replace('a5shop','shop-builder',$v['picName']);
