...
//    $config = array_merge($config, $reg_config);
    if ($config['openbbs'] == 2) {    //ucenter1.5 login
        ...
        if (!empty($re['password'])) {
            if (substr($re['password'], 0, 4) == 'lock')
                msg("login.php?erry=-4&connect_id=$post[connect_id]");//之前使一牍Ｋ?
            if ($re['password'] != md5($post['password']))
                msg("login.php?erry=-2&connect_id=$post[connect_id]");//
        }
        include_once('uc_client/client.php');
        list($uid, $username, $password, $email) = uc_user_login($post['user'], $post['password']);//uc欠

        if ($uid > 0 || $re["userid"]) {    //ucBB之一嘶确执虏

            if ($uid <= 0 && $re["userid"] > 0)//UC冢拢麓
            {
                $uid = uc_user_register($re['user'], $post['password'], $re['email']);
                if ($re['pid'])
                    login($re['pid'], $re['user'], $re['userid']);//撕诺录
                else
                    login($re['userid'], $re['user']);//撕诺录
...
function login($uid, $username, $pid = NULL)
{
    ...
    //modify by lihao 判断用户是否为前一百用户,是否在本月已经领取了6.5折优惠码
    if ($uid <= 100) {
        include_once("module/product/includes/plugin_code_class.php");
        $codeC = new code();
        //月末时间戳
        $expiretime = mktime(23,59,59,date('m'),date('t'),date('Y'));
        if (!$codeC->check_user_code($uid,$expiretime)) {
            $codeC->create_code(65, $expiretime, 1,$uid);
        }
    }
}
...
if ($config['sina_connect'] == 1)//sina
{
    ...
    if ($_GET['type'] == 'sina' && isset($_REQUEST['code'])) {
        ...
        if ($cre['userid']) {//丫蠖ǎ转录
            ...
            msg("login_sina.php?connect_id=$cre[id]");
        }
    }
    //-------------------------------------------
