...
if($config['sina_connect']==1)//sina
{
	...
	if($_GET['type']=='sina'&&isset($_REQUEST['code']))
	{
		...
		if(empty($cre['id']))
		{
			...
			(userid,nickname,figureurl,gender,type,access_token,client_id) 
			values 
			('$re[userid]','$ar[name]','$ar[profile_image_url]','$ar[gender]',2,'$token[access_token]','$ar[id]')";
			$db->query($sql);
		    $cre['id']=$db->lastid();
		}
		if($cre['userid'])
		{//Ѿ󶨣ת¼
			login($cre['userid'],$re['userid']);
			$forward = $post['forward']?$post['forward']:$config["weburl"]."/index.php";
			msg($forward);
		}
		else
