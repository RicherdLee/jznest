...
if (!file_exists(EMLOG_ROOT.'/content/plugins/sinav2/sinav2_token_conf.php'))
{
	...
	{
        // 验证state，防止伪造请求跨站攻击
        $sinav2_state = $_REQUEST['state'];
        if ( empty($sinav2_state) || $sinav2_state !== $_SESSION['sinav2_state'] ) {
            emMsg('非法请求！');
        }
        unset($_SESSION['sinav2_state']);
	...
	{
        $sinav2_state = uniqid( 'sinav2_', true);
        $_SESSION['sinav2_state'] = $sinav2_state;
        $sinav2_aurl = $sinav2_o->getAuthorizeURL( SINAV2_CALLBACK_URL , 'code', $sinav2_state, 'default', true );
	}
} 
else //auth done
{
...
if (!isset($_GET['oauth_token']) && !defined('SINAV2_ACCESS_TOKEN')): ?>
	<li><a href="<?php echo $sinav2_aurl ?>"><img src="../content/plugins/sinav2/t-login.png"></a></li>
<?php else:?>
<?php
...
<p><input name="input" type="submit" value="保存设置" /> <span><a href="http://t.sina.com" target="_blank">访问新浪微博&raquo;</a></span><span><a href="http://www.6jiji.com/suiyu/" target="_blank">访问插件作者博客&raquo;</a></span></p>
</div>
</form>
<?php
}
