...
if($config['domaincity']&&!empty($config['baseurl']))
{
	...
	{
		$sql="select con,con2,web_title,web_keyword,web_des,des,copyright,template,logo from ".DOMAIN." where top_domain='http://".$_SERVER['HTTP_HOST']."'";
		$db->query($sql);
		$do=$db->fetchRow();
		if(is_array($do))
		{
...
if(!empty($_COOKIE['AID'])&&empty($daid))
	$daid=$_COOKIE['AID'];
$domain=$daid?$daid:($dcid?$dcid:($dpid?$dpid:""));
//-----------------------------------------
//搜索词入库
if(!empty($_GET['key']))
{
